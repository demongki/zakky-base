...
case 'storyanime':
                ...
                wibutolol = await getBuffer(`https://api.dapuhy.xyz/api/anime/storyanime?apikey=IJbbr7Ssbs`)
                KirBotz.sendMessage(from, wibutolol, video, { quoted: freply })
                break
...
case 'igdl':
                    ...
                    ini_url = args[0]
                    ini_url = await fetchJson(`https://api.lolhuman.xyz/api/instagram?apikey=KirBotzzz01&url=${ini_url}`)
                    ini_url = ini_url.result
                    ini_type = image
                    if (ini_url.includes(".mp4")) ini_type = video
                    ini_buffer = await getBuffer(ini_url)
                    await KirBotz.sendMessage(from, ini_buffer, ini_type, { quoted: akira })
                    break

case 'igdl2':
                    if (args.length == 0) return reply(`Example: ${prefix + command} https://www.instagram.com/p/CJ8XKFmJ4al/?igshid=1acpcqo44kgkn`)
                    ini_url = args[0]
                    ini_url = await fetchJson(`https://api.lolhuman.xyz/api/instagram2?apikey=KirBotzzz01&url=${ini_url}`)
...
case 'quranaudio':
                ...
                surah = args.join(' ')
                halal = await getBuffer(`https://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=KirBotzzz01`)
                dp.sendMessage(from, halal, audio, { mimetype: 'audio/mp4', filename: `${command}.mp3`, quoted: akira})
                break
...
case 'asupan':
				...
				daptod = await getBuffer(`https://api.dapuhy.xyz/api/asupan/${command}?apikey=IJbbr7Ssbs`)
				KirBotz.sendMessage(from, daptod, video, {quoted: akira})
				break
...
//[❗] case by Bloodsword
                    reply(kirr.wait)
                teks = args.join(' ')
                dapuhy = await getBuffer(`https://api.dapuhy.xyz/api/maker/nulis?text=${teks}&apikey=IJbbr7Ssbs`)
                KirBotz.sendMessage(from, dapuhy, image, {quoted: akira})
                break
