...
case 'cerpen':
                ...
                case 'quotesdilan':
                    zakky = await fetchJson(`https://api.lolhuman.xyz/api/quotes/dilan?apikey=${LolKey}`)
                    reply(zakky.result)
                    break

                case 'quotesimage':
                    get_result = await getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${LolKey}`)
                    await KirBotz.sendMessage(from, get_result, image, { quotes: akira })
                    break
...
case 'storyanime':
                ...
                zakky = await getBuffer(`https://api.dapuhy.xyz/api/anime/storyanime?apikey=${Dapkey}`)
...
case 'quranaudio':
                ...
                surah = args.join(' ')
                zakky = await getBuffer(`https://api.lolhuman.xyz/api/quran/audio/${surah}?apikey=${LolKey}`)
...
//[❗] case by Bloodsword
                    reply(kirr.wait)
                teks = args.join(' ')
                zakky = await getBuffer(`https://api.dapuhy.xyz/api/maker/nulis?text=${teks}&apikey=${Dapkey}`)
                KirBotz.sendMessage(from, zakky, image, {quoted: akira})
                break
